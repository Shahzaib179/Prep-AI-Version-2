...
# Google Drive is optional.  The app can still deploy and use local uploads
# when the Google Drive helper is not present in the deployed repository.
try:
    from sources.google_drive import GoogleDriveSource
except ModuleNotFoundError as e:
    if not (str(e).startswith("No module named 'sources'") or
            str(e).startswith("No module named 'sources.google_drive'")):
        raise

    class GoogleDriveSource:
        def load(self, url):
            raise RuntimeError(
                "Google Drive support is not available in this deployment. "
                "Add sources/google_drive.py to the repository to enable it."
            )
